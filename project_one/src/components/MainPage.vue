    ...
    <v-layout align-space-around justify-space-around fill-height wrap class="px-8 pt-0 pb-0 mr-1">
      <v-flex md2 xs5 class="pl-1 d-flex justify-center">
        <v-card max-width="260" height="870" class="ml-0 pr-0 justify-space-between" color="grey lighten-3" tile flat>
          <ul class="pl-0">
            <div align="center">
              <v-btn @click.stop="dialog = true" color="grey lighten-1" class="my-2 mb-5 justify-center" width="180" depressed tile dark flat>Create
              </v-btn>
            ...
            <div>
              <v-list-group no-action sub-group class="px-0" :value="expandList1" disabled>
                ...
                <v-list-item v-show="showGFaceMangerInput" @mouseover="hover = true" @mouseleave="hover = false"
                  class="pl-12 ml-0" link>
                  <v-list-item-title>
                    <input @keyup.enter="onPlusAddItem1()" type="text" v-model="addItem1" placeholder="Add item"
                      style="width: 130px; background-color: whitesmoke;" />
                  </v-list-item-title>
                  <v-btn icon style="float:right" class="mb-1" color="black" depressed tile x-small
                    @click="onPlusAddItem1()">
                    <v-icon>mdi-plus-circle-outline</v-icon>
                  </v-btn>
                </v-list-item>

                <v-list-item v-for="(admin, index) in admins" :key="index" @mouseover="hover = true"
            ...
            <div>
              <v-list-group no-action sub-group class="px-0" :value="expandList2" disabled>
                <template v-slot:activator>
                  <v-list-item-content class="margin-right:5px;">
                    <v-list-item-title class="pr-0">
                      ...
                      <v-menu :offset-x="true" :closeOnContentClick="false">
                        <template v-slot:activator="{ on }">
                          <v-btn icon @click="onPlusButtonClick2" v-on="on" style="float:right" class="mb-1"
                            color="black" depressed tile small>
                            <v-icon>mdi-plus-circle-outline</v-icon>
                          </v-btn>
                        </template>
                ...
                <v-list-item v-show="showGFaceUserInput" @mouseover="hover = true" @mouseleave="hover = false"
                  class="pl-12 ml-0" link>
                  <v-list-item-title>
                    <input @keyup.enter="onPlusAddItem2()" type="text" v-model="addItem2" placeholder="Add item"
                      style="width: 130px; background-color: whitesmoke;" />
                  </v-list-item-title>
                  <v-btn icon style="float:right" class="mb-1" color="black" depressed tile x-small
                ...
                <v-list-item v-for="(crud, index) in cruds" :key="index" @mouseover="hover = true"
                  @mouseleave="hover = false" class="pl-12 ml-0" link>
                  <v-list-item-title>
                    <span v-show="!crud.edit">{{ crud.title }}</span>
                    <input @keyup.enter="this.blur()" type="text" v-model="crud.title" v-show="crud.edit"
                      style="width: 130px; background-color: whitesmoke;" />
                  </v-list-item-title>
